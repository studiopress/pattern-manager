...
?>
' . prepare_content( $pattern['title'], $pattern['content'], $text_domain ) . '
';
 ...
 * @return bool
 */
function prepare_content( $pattern_slug, $pattern_html, $text_domain ) {
	$pattern_html = addcslashes( $pattern_html, '\'' );
	$pattern_html = move_block_images_to_theme( $pattern_slug, $pattern_html );
	return $pattern_html;
}
...
function tree_shake_theme_images() {
	...
	foreach ( $patterns_in_theme as $pattern_data ) {
		...
		foreach ( $urls_found as $url_found ) {
			...
			$desired_destination_in_theme = str_replace( $backedup_images_url, $images_url, $local_path_to_image );
 ...
 */
function move_block_images_to_theme( $pattern_slug, $pattern_html ) {
