...
function construct_pattern_php_file_contents( $pattern, $text_domain ) {
	...
	$pattern['content'] = move_block_images_to_theme( $wp_filesystem, $pattern['content'] );
 ...
 */
function move_block_images_to_theme( WP_Filesystem_Base $wp_filesystem, string $pattern_html ) {
	if ( ! $wp_filesystem ) {
		return $pattern_html;
	}
