	...
	edit( { attributes, setAttributes } ) {
			...
			for( const patternName in patternManager?.patterns ) {
				...
				renderedPatternList.push(
					<button
						...
						onClick={() => {
							setAttributes( { slug: patternManager?.patterns[patternName].name } );
						}}
					>
						{ patternName }
						<PatternPreview
							...
							viewportWidth={ patternManager?.patterns[patternName].viewportWidth }
						/>
					</button>
				)
			}
		...
		if ( ! pattern ) {
			return <div>
				...
				{__( 'Selected pattern Not found in the theme', 'pattern-manager' )}
			</div>
		}
